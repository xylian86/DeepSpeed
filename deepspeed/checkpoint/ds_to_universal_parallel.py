...
def _extract_zero_shard_files(args, ds_checkpoint, temp_dir):
    ...
    size = comm.Get_size()
    if rank == 0:
        chunk_size = len(_3d_range_list) // size
        remainder = len(_3d_range_list) % size
        start_idx = 0
        for i in range(1, size):
            # Calculate chunk size for this process (add one extra item if within remainder)
            proc_chunk_size = chunk_size + (1 if i < remainder else 0)
            # Send this process's chunk
            chunk = _3d_range_list[start_idx:start_idx + proc_chunk_size]
            comm.send(chunk, dest=i)
            start_idx += proc_chunk_size
            
        # Master process takes its own chunk
        _3d_range_list = _3d_range_list[start_idx:]
        print(f"Master process {rank} has {len(_3d_range_list)} chunks to process")
        print(f"Master process {rank} has {_3d_range_list=}")
    else:
        # receive the work chunk from the master process
        _3d_range_list = comm.recv(source=0)
        print(f"Process {rank} has {len(_3d_range_list)} chunks to process")
        print(f"Process {rank} has {_3d_range_list=}")
